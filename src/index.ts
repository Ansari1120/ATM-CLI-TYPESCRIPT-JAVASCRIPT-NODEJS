...
import UserInput from "./interface/userInformation.js";
import ATMOperations from "./interface/available-operations.js";
...
const welcomeMessage = async () => {
  const rainbowTitle = chalkAnimation.rainbow(
    "This ATM Project is Developed By Ahmed Ali Ansari PIAIC 171908"
  );
  await sleep();
  rainbowTitle.stop();
};
...
var promptLogin = async (): Promise<void> => {
...
const CheckIns = async (id: any, pass: number) : Promise<void> => {
    const todoBuffer = fs.readFileSync(__dirname + "/todos.json");
        ...
        const check1 = arrayOfUsersIds.find((el:any) => { return el.count == userInfo.uid});
        const check2 =  arrayOfUsersIds.find((el:any) => {return el.pass == userInfo.userPin})
        if (check1 && check2) {
             name1 = arrayOfUsersIds.filter((x:any) => x.count == id).map((x:any) => x.name);
...
async function operations_to_do(uid:string,username:string) {
