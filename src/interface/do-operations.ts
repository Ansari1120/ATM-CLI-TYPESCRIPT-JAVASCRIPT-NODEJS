...
import ATMOperations from "src/interface/available-operations.js";
...
export const performOperations = async (
  ...
  const Transfer = async (): Promise<number> => {
    const { amount } = await inquirer.prompt([
      {
        type: "string",
        name: "amount",
  ...
  const Account = async (): Promise<number> => {
  ...
  const Fast_cash = async (): Promise<number> => {
    const { amount } = await inquirer.prompt([
      {
        name: "amount",
  ...
  switch (operation) {
    ...
    case "Withdraw":
      ...
      break;
    case "Funds Transfer":
      const Transfer_amount: number = await Transfer();
      const Account_no: number = await Account();
      ...
      spinner.success({
        text: `${chalk.bgBlue(
          ...
          Account No: ${chalk.bgCyan(
            Math.floor(Math.random() * 10000000000000).toString(10)
          )}
          Account type : ${chalk.bgBlue(
            "Current"
